...
export default function EditCollection() {
    const { collection, links: existingLinks } = usePage<{
    ...
    function deleteLink(link: LinkType) {
        router.delete(
            LinkController.destroy.url({ collection: collection.id, link: link.id }),
            { preserveScroll: true },
        );
    }
    ...
    setLayoutProps({
        breadcrumbs: [
            ...
            { title: 'Modifier', href: collections.edit.url(collection) },
        ],
    });
    ...
    return (
        <>
            <Head title="Modifier la collection" />
            <div className="container mx-auto max-w-xl space-y-6 px-8 py-4">
                <h1 className="text-2xl font-semibold">Modifier la collection</h1>

                {/* POST + _method spoofing required for file uploads with PATCH */}
                <Form
                    action={CollectionController.update.url(collection)}
                    method="post"
                    className="space-y-5"
                >
                    {({ processing, errors }) => (
                            ...
                            <div className="grid gap-2">
                                ...
                                <div className="flex items-center gap-3">
                                    <input
                                        ...
                                        name="color"
                                        type="color"
                                        defaultValue={collection.color ?? '#6366f1'}
